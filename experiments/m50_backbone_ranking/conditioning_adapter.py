...
def _apply_diagonal_transform(
    centered: np.ndarray,
    diagonal_scale: np.ndarray,
) -> np.ndarray:
    """Apply a diagonal transform without materializing a dense matmul.

    M50 is deliberately diagonal.  Multiplying each feature in place of the
    equivalent ``centered @ np.diag(scale)`` avoids an O(n*d**2) operation at
    backbone dimensions while retaining the same float64 arithmetic for each
    feature.  The helper is kept private so it cannot become public API.
    """

    return np.asarray(centered * diagonal_scale, dtype=np.float64)
...
class _FittedConditioner:
    ...
    def __init__(
        ...
        *,
        ...
        self._diagonal_scale: Optional[np.ndarray] = None
        if (
            self.transform_.ndim == 2
            and self.transform_.shape[0] == self.transform_.shape[1]
        ):
            # Every conditioned estimator in this frozen adapter constructs a
            # diagonal transform.  Do not scan or subtract a dense d-by-d
            # matrix here: the fitted transform is retained for provenance,
            # while application uses only its diagonal vector.
            self._diagonal_scale = _readonly_float(np.diag(self.transform_))
    ...
    def transform(self, X: np.ndarray) -> np.ndarray:
        ...
        if self._diagonal_scale is not None:
            return _apply_diagonal_transform(centered, self._diagonal_scale)
