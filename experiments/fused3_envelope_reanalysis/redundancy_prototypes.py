"""OI-aligned second-own prototype diagnostic for squared FUSED3 states."""
...
STUDY = "fused3_confirmation_v2_aircraft_redundancy_prototype_diagnostic"
STAGE = "post_outcome_exploratory_diagnostic"
STATUS = "descriptive_only_no_promotion"
DATASET_ID = "torchvision_fgvc_aircraft"
ANCHOR_STRENGTH = 0.1
RANK_COMPONENTS = ("best_own_win_rate", "second_own_win_rate", "exact_oi")


def _stage_id(base_state: str, epoch: int) -> str:
    if epoch == 0:
        return base_state
    return f"{base_state}-OILVQ{epoch}"
...
def redundancy_lvq_epoch(
    ...
    *,
    ...
    """Move the second-own threshold toward OI-violating rows."""

    matrix, labels = _validate_matrix_target(values, target)
    prototypes = np.asarray(centers, dtype=np.float32)
    anchors = np.asarray(anchor_centers, dtype=np.float32)
    ...
    if anchors.shape != prototypes.shape:
        raise ValueError("anchor_centers must match centers")
    ...
    if not isinstance(anchor_strength, float) or not 0.0 <= anchor_strength < 1.0:
        raise ValueError("anchor_strength must be a float in [0, 1)")
    ...
    if any(len(ids) < 2 for ids in ids_by_class.values()):
        raise ValueError("OI-aligned LVQ requires two prototypes per class")

    scores = _score_kernel(matrix, prototypes, feature_weights)
    second_choice = np.empty(len(matrix), dtype=np.int64)
    wrong_choice = np.empty(len(matrix), dtype=np.int64)
    second_score = np.empty(len(matrix), dtype=np.float32)
    ...
    for label in dict.fromkeys(labels.tolist()):
        ...
        own_order = np.argsort(scores[rows][:, own_ids], axis=1, kind="stable")
        local_second = own_order[:, -2]
        local_wrong = np.argmax(scores[rows][:, wrong_ids], axis=1)
        second_choice[rows] = own_ids[local_second]
        wrong_choice[rows] = wrong_ids[local_wrong]
        second_score[rows] = scores[rows, second_choice[rows]]
        wrong_score[rows] = scores[rows, wrong_choice[rows]]
    active = wrong_score > second_score
    ...
    if np.any(active):
        ...
        active_second = second_choice[active]
        active_wrong = wrong_choice[active]
        np.add.at(
            delta,
            active_second,
            active_values - prototypes[active_second].astype(np.float64),
        )
        np.add.at(
            delta,
            active_wrong,
        ...
        np.add.at(counts, active_second, 1)
        np.add.at(counts, active_wrong, 1)
        changed = counts > 0
        candidate = updated[changed] + (
            learning_rate * delta[changed] / counts[changed, None]
        )
        updated[changed] = anchors[changed].astype(np.float64) + (
            1.0 - anchor_strength
        ) * (candidate - anchors[changed].astype(np.float64))
    output = np.asarray(updated, dtype=np.float32)
    output.setflags(write=False)
    displacement = np.linalg.norm(
        output.astype(np.float64) - prototypes.astype(np.float64), axis=1
    )
    anchor_displacement = np.linalg.norm(
        output.astype(np.float64) - anchors.astype(np.float64), axis=1
    )
    return output, {
        "algorithm": "batch_second_own_oi_violation_lvq_v1",
        ...
        "mean_changed_prototype_step": (
            0.0
            if not np.any(counts)
            else float(np.mean(displacement[counts > 0]))
        ),
        "max_prototype_step": float(np.max(displacement, initial=0.0)),
        "max_distance_from_anchor": float(np.max(anchor_displacement, initial=0.0)),
        "weights_relearned": False,
        "prototype_owners_changed": False,
        "prototype_count_changed": False,
        "best_own_prototype_attracted": False,
        "second_own_prototype_attracted": True,
        "nearest_rival_prototype_repelled": True,
    }


def crossfit_redundancy_state(
    values: Any,
    target: Any,
    *,
    seed: int,
    ...
    """Derive anchored OI-LVQ epoch 0/1/3 states inside frozen folds."""
    ...
    for fold, (train, holdout) in enumerate(folds):
        ...
        anchors = np.array(selector.centers_, dtype=np.float32, copy=True)
        ...
        for epoch in range(0, max(EPOCHS) + 1):
            ...
            if epoch == max(EPOCHS):
                break
            centers, last_update = redundancy_lvq_epoch(
                update_values,
                update_labels,
                centers=centers,
        ...
        if not np.array_equal(selector.owners_, owners_before):
            raise RuntimeError("OI-LVQ diagnostic mutated prototype owners")
        if not np.array_equal(selector.weights_, weights_before):
            raise RuntimeError("OI-LVQ diagnostic mutated relevance weights")
...
def analyze_redundancy_prototypes(
        ...
        for replicate, seed in enumerate(statistics.REPLICATE_SEEDS):
            for budget in statistics.BUDGETS:
                ...
                for base_state, square_count in BASE_STATES:
                    result = crossfit_redundancy_state(
    ...
    if len(state_rows) != 600 or len(fold_rows) != 3000:
        raise RuntimeError("OI-LVQ diagnostic grid is incomplete")
    ...
    return {
        ...
        "update_contract": {
            ...
            "active_rows": "nearest rival score strictly greater than second-own score",
            "own_update": "move second-nearest own prototype toward active row",
            "rival_update": "move nearest rival prototype away from active row",
            "anchor": "shrink each changed prototype toward its epoch-zero center",
            "weights_relearned": False,
...
def _report(summary: Mapping[str, Any]) -> str:
    lines = [
        "# Aircraft OI-aligned redundancy-prototype diagnostic",
        "",
    ...
    lines.extend(
        [
            "",
            "OI-LVQ1/OI-LVQ3 update only the second-own threshold and nearest rival on training-fold violations, with an epoch-zero anchor. Feature weights, owners/counts, and exact OI scoring remain fixed. No row authorizes promotion or reselection.",
            "",
        ]
    )
    return "\n".join(lines)
...
def write_bundle(
    ...
    summary = analyze_redundancy_prototypes(
    ...
    for name, descriptor in manifest["files"].items():
        if _sha256(destination / name) != descriptor["sha256"]:
            raise RuntimeError("written OI-LVQ artifact hash mismatch")
    return summary
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    write_bundle(args.input, args.registry_root, args.squared_artifact, args.output)
    return 0
